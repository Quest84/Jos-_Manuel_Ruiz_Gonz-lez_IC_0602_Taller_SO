...
int main()
{
	...
	struct Sistema menuActualizarDatos(struct Sistema x, int indiceTemp)
	{
		printf("\t--> Cliente [%i] --> Nombre: %s --> Sexo: [%s] --> Sueldo: [%f]", x.cliente[indiceTemp].id_cliente,
			   x.cliente[indiceTemp].nombre, x.cliente[indiceTemp].sexo, x.cliente[indiceTemp].sueldo);
		do
		{
			system("clear");
			fflush(stdin);
			...
			case UPD_NOMBRE:
				...
				fgets(x.cliente[indiceTemp].nombre, sizeof(x.cliente[indiceTemp].nombre), stdin);
				char *ptr = strchr(x.cliente[indiceTemp].nombre, '\n');
				if (ptr)
				{
					*ptr = '\0';
				}
				printf("\t--> Cliente [%i] --> Nombre: %s --> Sexo: [%s] --> Sueldo: [%f]", x.cliente[indiceTemp].id_cliente, x.cliente[indiceTemp].nombre, x.cliente[indiceTemp].sexo, x.cliente[indiceTemp].sueldo);
				paisValidado = 0;
				while (getchar() != '\n')
					;
				break;
			...
			case UPD_SEXO:
				...
				char nuevoSexo[2];
				scanf("%c", nuevoSexo);
				strcpy(x.cliente[indiceTemp].sexo, nuevoSexo);

				printf("\t--> Cliente [%i] --> Nombre: %s --> Sexo: [%s] --> Sueldo: [%f]",
					   x.cliente[indiceTemp].id_cliente, x.cliente[indiceTemp].nombre, x.cliente[indiceTemp].sexo, x.cliente[indiceTemp].sueldo);
				paisValidado = 0;
				while (getchar() != '\n')
					;
				break;
	...
	while (1)
	{
		...
		case UPD_CLIENTE:
			...
			do
			{
				...
				if (ValidarIndiceCliente(Sys[resultado_pais], indiceTemp) == 1)
				{
					Sys[resultado_pais] = menuActualizarDatos(Sys[resultado_pais], indiceTemp);
				}
			} while (paisValidado == 0);
